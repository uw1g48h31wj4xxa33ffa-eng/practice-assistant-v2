...
import { generateMockRiskItems } from '@/lib/ai/mockRiskAnalyzer';

export default function RiskAnalysisPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const riskItems = currentCase.riskItems || [];
  
  const totalItems = riskItems.length;
  const verifiedCount = riskItems.filter(i => i.status === 'verified').length;
  const notApplicableCount = riskItems.filter(i => i.status === 'not_applicable').length;
  const needsRevisionCount = riskItems.filter(i => i.status === 'needs_revision').length;
  const uncheckedCount = riskItems.filter(i => i.status === 'unchecked').length;
  ...
  const handleNextStep = () => {
    if (hasIncomplete) {
      const confirmMsg = "未確認または要修正のリスクが残っています。\nこのまま次の工程（対応方針）へ進みますか？";
      if (!window.confirm(confirmMsg)) {
        return;
      }
    }
    updateCase(caseId, { progressStatus: 'action_plan' });
    router.push(`/cases/${caseId}/action-plan`);
  };

  const handleGenerate = async () => {
    setIsAnalyzing(true);
    try {
      const newItems = await generateMockRiskItems();
      updateCase(caseId, { riskItems: newItems });
    } finally {
      setIsAnalyzing(false);
    }
  };

  const handleStatusChange = (itemId: string, newStatus: RiskItem['status']) => {
    const newItems = riskItems.map(item => 
    ...
    if (isCompleted(newStatus) || newStatus === 'needs_revision') {
      const currentIndex = riskItems.findIndex(i => i.id === itemId);
      ...
      setTimeout(() => {
        if (nextTarget) {
          const element = document.getElementById(`risk-card-${nextTarget.id}`);
          if (element) {
            element.scrollIntoView({ behavior: 'smooth', block: 'center' });
          }
        } else {
          // All complete
          const element = document.getElementById('risk-completion-area');
          if (element) {
            element.scrollIntoView({ behavior: 'smooth', block: 'center' });
          }
        }
  ...
  const renderContextualButtons = (item: RiskItem) => {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-slate-800">リスク整理</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="p-5">
              ...
              <div className="pt-5 border-t border-slate-100">
                <button
                  ...
                  className="w-full py-3 bg-rose-50 hover:bg-rose-100 text-rose-700 font-bold rounded-xl transition-all flex items-center justify-center gap-2 border border-rose-200 disabled:opacity-50"
                >
                  {isAnalyzing ? (
                    <>
                      <svg className="animate-spin h-5 w-5 text-rose-700" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="flex items-center justify-between mb-2">
            <h3 className="font-bold text-slate-800 text-lg">要確認リスク一覧</h3>
          ...
          {riskItems.length === 0 ? (
            <div className="bg-white border border-slate-200 rounded-xl p-12 text-center shadow-sm">
              ...
              <h3 className="text-lg font-bold text-slate-700 mb-2">リスクデータがありません</h3>
              <p className="text-slate-500 mb-6 text-sm">左側の「AIで整理する」ボタンをクリックして、<br />これまでの情報からリスク候補を抽出してください。</p>
            </div>
          ) : (
              ...
              {riskItems.map(item => (
                <div 
                  ...
                  className={`bg-white border rounded-xl p-5 shadow-sm transition-all ${
                    ...
                    item.riskLevel === 'high' ? 'border-rose-300 shadow-md bg-rose-50/10' :
                  ...
                  <div className="flex items-start justify-between gap-4 mb-3">
                    <div className="flex items-start gap-3">
                      {item.status === 'verified' ? (
                        ...
                        <div className="w-6 h-6 rounded-full bg-rose-100 text-rose-600 flex items-center justify-center shrink-0 mt-0.5 animate-pulse">
                          <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" /></svg>
                        </div>
                      ) : (
                      ...
                      <div>
                        <div className="flex flex-wrap items-center gap-2 mb-1">
                          ...
                          {item.riskLevel === 'high' && <span className="text-[10px] font-bold text-white bg-rose-500 px-1.5 py-0.5 rounded shadow-sm">高リスク</span>}
                          {item.riskLevel === 'medium' && <span className="text-[10px] font-bold text-slate-700 bg-amber-200 px-1.5 py-0.5 rounded">中リスク</span>}
                          {item.likelihood === 'high' && <span className="text-[10px] font-bold text-slate-500 border border-slate-200 px-1.5 py-0.5 rounded bg-white">発生確率:高</span>}
                          {item.impact === 'high' && <span className="text-[10px] font-bold text-slate-500 border border-slate-200 px-1.5 py-0.5 rounded bg-white">影響度:高</span>}
                        </div>
                        <h4 className="font-bold text-slate-800 text-[15px] sm:text-base leading-snug">{item.title}</h4>
                      </div>
                    </div>
                  </div>
                  
                  <div className="pl-9 mb-4 space-y-3">
                    <div>
                      <p className="text-xs font-bold text-slate-500 mb-1">リスク内容</p>
                      <p className="text-sm text-slate-700 bg-slate-50 p-3 rounded-lg border border-slate-100 leading-relaxed">{item.summary}</p>
                    </div>
                    {item.countermeasure && (
                      <div>
                        <p className="text-xs font-bold text-slate-500 mb-1">対応策案</p>
                        <p className="text-sm text-slate-700 bg-blue-50/50 p-3 rounded-lg border border-blue-100 leading-relaxed">{item.countermeasure}</p>
                      </div>
                    )}
      ...
      <div id="risk-completion-area" className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-8 mt-8 border-t border-slate-200">
        ...
        <button
          ...
          className="w-full sm:w-auto px-8 py-3 bg-rose-600 hover:bg-rose-700 text-white font-bold rounded-xl transition-all shadow-sm hover:shadow flex items-center justify-center gap-2"
        >
          <span>次の工程へ進む</span>
          <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
