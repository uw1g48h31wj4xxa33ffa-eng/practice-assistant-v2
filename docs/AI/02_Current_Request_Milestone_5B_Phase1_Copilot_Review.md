...
### 5.1 Approved dependency decision

For JSON Schema Draft 2020-12 validation, adding the following runtime dependencies is approved:
...
- use the Draft 2020-12-specific Ajv class/import (`Ajv2020`);
- do not use the default Draft-07 Ajv instance for these schemas;
- enable strict validation;
- register only the formats actually required by the Profile schemas;
- keep TypeScript types, JSON Schemas, fixtures, and tests aligned;
- document the dependency decision and validator configuration in `docs/AI/04_Decisions.md`;
- include `package-lock.json` changes generated by the package manager;
- do not add unrelated validation packages.

The canonical date-period convention is approved as:
...
Overlapping active periods for the same Profile identity must be rejected as ambiguous.

### 5.2 Canonical naming decision

Use the common metadata names already defined by this Request:
...
Do not introduce `profileId` or `profileVersion` aliases in this phase unless an existing repository contract requires them. If such a conflict exists, stop and report it before implementation.

### 5.3 Multi-AI review policy

GitHub Copilot is the code-diff review actor for this phase.

Role separation:

```text
ChatGPT
- architecture and Request definition
- specification/governance review
...
- type-safety, boundary-condition, error-handling, security, and test-gap review

Human
- final review and approval
```

Copilot comments are advisory evidence, not human approval. Gemini must not apply a Copilot suggestion automatically without checking it against this Request, the Master, tests, and repository facts.
...
## 11. Gate 5 — Diff Audit, Branch, Pull Request, Copilot Review, Merge, and Push

Direct implementation commits to `main` are not authorized for this phase.

After Gates 1–4 pass:

1. create or use the approved Milestone 5-B working branch;
2. audit all unstaged changes;
3. stage explicit approved paths only;
4. audit the staged diff;
5. commit the implementation;
6. push the working branch;
7. create a Pull Request targeting `main`;
8. request GitHub Copilot Code Review;
9. evaluate every Copilot comment against this Request and repository facts;
10. fix valid findings only;
11. rerun all affected tests, verification, build, and changed-file lint;
12. request Copilot re-review when code materially changes;
13. update machine evidence and numbered Markdown assets;
14. obtain explicit human approval before merge;
15. merge through the approved repository workflow;
16. verify `main` and the remote state after merge.
...
The Pull Request review must explicitly cover:

1. TypeScript types and JSON Schema consistency
2. Draft 2020-12 validator configuration
3. use of `Ajv2020`
4. unknown/additional property rejection policy
5. `effectiveFrom` inclusive boundary
6. `effectiveTo` exclusive boundary
7. overlap and ambiguity rejection
8. status handling for draft/deprecated/archived
9. duplicate identity/version rejection
10. validation before cache insertion
11. typed errors and error propagation
12. unsafe assertions, `any`, and silent fallback
13. source hash/checksum handling
14. regression risk to existing Word/document engines
15. missing tests and untested branches

Copilot findings must be recorded in the implementation report as:
...
Each rejected or not-applicable finding must include a concise reason.

Copilot must not be recorded as:
...
Required final state after approved merge:

```text
main HEAD = origin/main
working tree clean
Pull Request merged
Copilot findings resolved or dispositioned
human approval recorded
```

If final documentation or audit-log updates occur after the implementation commit or review cycle, commit and push them explicitly on the branch, update the Pull Request, rerun required verification, and re-verify the final state after merge. Do not report a clean repository before the final append/update is committed and merged.
...
- approved files only are committed on the working branch;
- a Pull Request targeting `main` is reviewed by GitHub Copilot;
- all Copilot findings are resolved or dispositioned;
- explicit human approval is recorded before merge;
- the Pull Request is merged through the approved workflow;
- `main HEAD = origin/main`;
