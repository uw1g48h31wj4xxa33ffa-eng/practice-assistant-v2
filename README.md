...
- **補助金業務フロー**: 補助金関連案件について、以下の一連のフローを実装済みです。
  - 公募要項整理
  - 必要資料整理
  - スケジュール管理
  - AI検証・エビデンス
...
> **重要**: 本プロジェクトに関する補足
> - 現時点ではデモ用途・ポートフォリオ用途のアプリケーションです。
> - AI API / OCR / 外部DB / RAG などの外部システムとは未接続です。
> - AI処理はすべてモック生成関数によって代替しています。
> - `localStorage` を利用したフロントエンド完結型のアーキテクチャです。
> - PCおよびスマホの両表示に対応しています。
> - 実運用に移行する際は、バックエンド構築、認証、DB連携、権限管理、ログ管理等が必要となります。
